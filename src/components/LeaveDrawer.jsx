...
export function LeaveDrawer({ open, onClose, employee, existingPeriods, onApplied }) {
  ...
  const nextPeriodNumber = [1, 2, 3].find((n) => !usedPeriodNumbers.has(n)) ?? null
  ...
  const canSubmit =
    !submitting && startDate && endDate && endDate >= startDate && nextPeriodNumber && employee
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <div className="relative flex h-full w-full max-w-md flex-col bg-surface shadow-panel transition-transform sm:w-[420px]">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <h2 className="font-display text-base font-semibold text-ink">Apply for Leave</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-5 py-5">
          {success ? (
            <div className="rounded-lg border border-success/30 bg-success-tint p-4">
              <p className="text-sm font-semibold text-success">✓ Leave Successfully Added</p>
              <p className="mt-2 text-sm text-ink">
                {formatLong(success.startDate)} → {formatLong(success.endDate)}
              </p>
              <p className="text-sm text-ink-muted">
                {success.duration} {success.duration === 1 ? 'Day' : 'Days'} · Period {success.periodNumber}
              </p>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {employee && (
                <p className="text-sm text-ink-muted">
                  Applying as <span className="font-medium text-ink">{employee.name}</span>
                </p>
              )}
              ...
              <p className="text-xs text-ink-muted">
                Leave periods used: {periodsUsed} / 3
              </p>
